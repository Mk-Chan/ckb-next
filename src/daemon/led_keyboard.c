...
int savergb_kb(usbdevice* kb, lighting* light, int mode){
    if(kb->fwversion >= 0x0120 || IS_V2_OVERRIDE(kb)){
        uchar data_pkt[12][MSG_SIZE] = {
            ...
            { CMD_SET, FIELD_KB_HWCOL, 0x03, 0x01, 0x01, mode + 1, COLOR_RED},
            // Green
            { CMD_WRITE_BULK, 0x01, 60, 0 },
            { CMD_WRITE_BULK, 0x02, 60, 0 },
            { CMD_WRITE_BULK, 0x03, 24, 0 },
            { CMD_SET, FIELD_KB_HWCOL, 0x03, 0x01, 0x01, mode + 1, COLOR_GREEN },
            // Blue
            { CMD_WRITE_BULK, 0x01, 60, 0 },
            { CMD_WRITE_BULK, 0x02, 60, 0 },
            { CMD_WRITE_BULK, 0x03, 24, 0 },
            { CMD_SET, FIELD_KB_HWCOL, 0x03, 0x01, 0x01, mode + 1, COLOR_BLUE }
        };
        makergb_full(light, data_pkt);
        if(!usbsend(kb, data_pkt[0], 12))
            return -1;
        if (IS_STRAFE(kb)){ // end save
            uchar save_end_pkt[MSG_SIZE] = { CMD_SET, FIELD_KB_HWCOL, 0x04, 0x01, 0x01 };
        ...
        uchar data_pkt[5][MSG_SIZE] = {
            ...
            { CMD_WRITE_BULK, 0x03, 60, 0 },
            { CMD_WRITE_BULK, 0x04, 36, 0 },
            { CMD_SET, FIELD_KB_HWCOL, 0x02, 0x00, 0x01, mode + 1 }
        };
...
int loadrgb_kb(usbdevice* kb, lighting* light, int mode){
    if(kb->fwversion >= 0x0120 || IS_V2_OVERRIDE(kb)){
        uchar data_pkt[12][MSG_SIZE] = {
            { CMD_GET, FIELD_KB_HWCOL, 0x03, 0x01, 0x01, mode + 1, COLOR_RED },
            { CMD_READ_BULK, 0x01, 60, 0 },
            { CMD_READ_BULK, 0x02, 60, 0 },
            { CMD_READ_BULK, 0x03, 24, 0 },
            { CMD_GET, FIELD_KB_HWCOL, 0x03, 0x01, 0x01, mode + 1, COLOR_GREEN },
            { CMD_READ_BULK, 0x01, 60, 0 },
            { CMD_READ_BULK, 0x02, 60, 0 },
            { CMD_READ_BULK, 0x03, 24, 0 },
            { CMD_GET, FIELD_KB_HWCOL, 0x03, 0x01, 0x01, mode + 1, COLOR_BLUE },
            { CMD_READ_BULK, 0x01, 60, 0 },
            { CMD_READ_BULK, 0x02, 60, 0 },
            { CMD_READ_BULK, 0x03, 24, 0 },
        };
        uchar in_pkt[4][MSG_SIZE] = {
            { CMD_GET, FIELD_KB_HWCOL, 0x03, 0x01 },
            { CMD_READ_BULK, 0x01, 60, 0 },
            { CMD_READ_BULK, 0x02, 60, 0 },
            { CMD_READ_BULK, 0x03, 24, 0 },
        };
        ...
        uchar data_pkt[5][MSG_SIZE] = {
            { CMD_GET, FIELD_KB_HWCOL, 0x02, 0x01, 0x01, mode + 1, 0 },
            { CMD_READ_BULK, 0x01, 60, 0 },
            { CMD_READ_BULK, 0x02, 60, 0 },
